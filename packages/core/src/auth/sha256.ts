...
/** Small dependency-free SHA-256 implementation for runtimes without Web Crypto. */
export function sha256Bytes(data: Uint8Array): Uint8Array {
  ...
  for (let offset = 0; offset < padded.length; offset += 64) {
    ...
    for (let index = 0; index < 64; index += 1) {
      ...
      const temporary1 = (h + upperSigma + choose + ROUND_CONSTANTS[index]! + schedule[index]!) >>> 0;
...
export function hmacSha256Bytes(key: Uint8Array, data: Uint8Array): Uint8Array {
  const normalizedKey = key.length > BLOCK_SIZE ? sha256Bytes(key) : key;
  ...
  return sha256Bytes(concatenate(outerPad, sha256Bytes(concatenate(innerPad, data))));
}

export function pbkdf2Sha256Bytes(
  password: Uint8Array,
  salt: Uint8Array,
  iterations: number,
  outputLength = 32,
  ...
  if (!Number.isInteger(outputLength) || outputLength <= 0) {
    throw new Error("PBKDF2 output length must be a positive integer");
  }
  ...
  for (let block = 1; block <= blockCount; block += 1) {
    ...
    for (let round = 1; round < iterations; round += 1) {
      ...
      for (let index = 0; index < accumulated.length; index += 1) {
        accumulated[index] = accumulated[index]! ^ intermediate[index]!;
      }
    }
