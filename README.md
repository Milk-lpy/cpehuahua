...
- iPhone 优先的登录界面；“记住密码”和“自动登录”可分别设置，认证成功后自动进入概览并启动实时轮询
- 深色/白天两套持久化主题，可在登录页、页头或设置页切换
- 六个主导航页面：概览、控制、锁频、参数、短信、设置；Probe 仅保留在 Bridge 诊断接口中，不在生产导航显示
- 动态 PCC/SCC/Neighbor 展示；首页副载波严格来自 `/api/device/seccellinfo`，展示层会滤掉与 PCC 身份完全相同的镜像项，不会把邻区伪装成 SCC
- 流量统计、套餐/签约速率本地设置、在线终端、短信邮箱和 PWA 离线壳
- “设备日志”独立详情页及概览摘要；同一轮采样的变化合并成时间线节点，并保留检测窗口、前值/确认值、来源及前后快照上下文
...
- 短信收件箱/发件箱、发送、已读和删除；移动数据开关、网络模式、4G/5G Band 锁定、
  网络重连、流量清零、设备重启；在线终端详情和兼容性 MAC 过滤断网

本轮没有宣称生产级全量控制支持；用户可显式配置自己的低负载 HTTPS 用户路径探测，
也可主动选用 Apple 联网检测地址。自动升级、定时重启、灯光、MLO、PMF、NFC、VPN、
双宽带、终端限速/协议重命名、WLAN/APN、恢复出厂和关机仍缺少本机安全写入证据，
因此不出现在正式控制界面中，也不进入 Bridge action 白名单。终端显示名称仅保存在
浏览器本地，不会写进 CPE。
