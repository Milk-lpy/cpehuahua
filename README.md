...
一批只读 endpoint；短信、锁频、移动数据、终端过滤、自动升级和重启写入仍需在该 H168 上逐项回读。没有实机返回证据
...
- iPhone 优先的深色登录界面；“记住密码”和“自动登录”可分别设置，认证成功后自动进入概览并启动实时轮询
- 六个主页面：概览、控制、锁频、参数、短信、设置；Probe 仅保留在 Bridge 诊断接口中，不在生产导航显示
- 动态 PCC/SCC/Neighbor 展示；首页副载波严格来自 `/api/device/seccellinfo`，不会把邻区伪装成 SCC
- 流量统计、套餐/签约速率本地设置、在线终端、短信邮箱、WLAN 只读配置和 PWA 离线壳
- “设备日志”独立详情页及概览摘要，记录小区/PCI/信号/连接状态变化和可追溯上下文
...
- 短信收件箱/发件箱、发送、已读和删除；移动数据开关、网络模式、4G/5G Band 锁定、
  网络重连、流量清零、设备重启和自动升级候选写入；在线终端详情和兼容性 MAC 过滤断网

本轮没有宣称生产级全量控制支持；用户可显式配置自己的低负载 HTTPS 用户路径探测，
也可主动选用 Apple 联网检测地址。自动升级只有在 H168 实际返回配置端点后才开放，且
写入后必须回读；定时重启、灯光、MLO、PMF、NFC、VPN、双宽带、终端限速/协议重命名、
WLAN/APN、恢复出厂和关机仍缺少本机安全写入证据，界面会显示只读或不可用，不会用
猜测按钮代替。终端显示名称仅保存在浏览器本地，不会写进 CPE。
