...
function DevicePage({ snapshot, cached, onClearCache }: { snapshot: CpeSnapshot; cached: boolean; onClearCache: () => void }) {
  return <><section className="soft-panel"><SectionTitle eyebrow="Device" title="设备信息" /><DetailList items={[["设备型号", snapshot.device.model], ["产品名称", snapshot.device.productName], ["开机时长", formatUptime(snapshot.device.uptimeSeconds)], ["硬件版本", snapshot.device.hardwareVersion], ["软件版本", snapshot.device.firmware], ["Web UI 版本", snapshot.device.webUiVersion], ["参数版本", snapshot.device.parameterVersion]]} /></section><section className="soft-panel"><SectionTitle eyebrow="Connected devices" title="在线终端" badge={snapshot.clients.length} /><div className="client-list">{snapshot.clients.length ? snapshot.clients.map((client, index) => <article className="client-card" key={client.id ?? `${client.name}-${index}`}><div className="client-icon" aria-hidden="true">{client.manufacturer === "Apple" ? "●" : "◆"}</div><div><h3>{client.name ?? client.hostName ?? `终端 ${index + 1}`}</h3><p>{[client.manufacturer, client.deviceType, client.frequency, client.ssid].filter(Boolean).join(" · ") || "设备未返回描述"}</p><span>在线 {formatUptime(client.associatedSeconds)}</span></div></article>) : <Empty text="尚未读取到在线 WLAN 终端。" />}</div></section><section className="soft-panel"><SectionTitle eyebrow="Messages" title="短信概览" badge={snapshot.messaging.unread ?? "—"} /><div className="message-grid"><span><b>{snapshot.messaging.inbox ?? "—"}</b><small>收件箱</small></span><span><b>{snapshot.messaging.outbox ?? "—"}</b><small>发件箱</small></span><span><b>{snapshot.messaging.unread ?? "—"}</b><small>未读</small></span><span><b>{snapshot.messaging.capacity === null ? "—" : Math.max(0, snapshot.messaging.capacity - (snapshot.messaging.inbox ?? 0))}</b><small>剩余容量</small></span></div><p className="panel-note">只读取邮箱计数，不读取、发送或删除短信内容。{snapshot.messaging.storageFull === true ? " 当前存储已满。" : ""}</p></section><section className="soft-panel"><SectionTitle eyebrow="Network identity" title="网络身份" /><DetailList items={[["运营商", snapshot.connection.operatorName], ["PLMN", snapshot.connection.plmn], ["Huawei 状态码", snapshot.connection.cellularStatusCode], ["模式", snapshot.connection.radioMode], ["SA / NSA", snapshot.connection.saNsa], ["Cell ID", snapshot.radio.cellId], ["TAC", snapshot.radio.tac]]} /></section><section className="soft-panel privacy-card"><SectionTitle eyebrow="Privacy" title="本地数据" /><p>页面只保存规范化快照与事件，不保存密码、Session、Token、IP/MAC 或 RAW XML。</p>{cached && <button className="danger-soft-button" type="button" onClick={onClearCache}>清除本地快照</button>}</section></>;
}
