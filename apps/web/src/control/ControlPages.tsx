...
const LTE_BANDS = [1, 3, 5, 7, 8, 20, 28, 38, 40, 41, 42, 43, 71] as const;
...
export function ControlPage({ client }: { client: H168ControlClient }) {
  ...
  async function setMobileData(enabled: boolean) {
    ...
    try { await client.execute("network.mobile-data", { enabled }); setNotice(enabled ? "移动数据已开启" : "移动数据已关闭"); await refresh(); }
...
export function BandLockPage({ client, snapshot }: { client: H168ControlClient; snapshot: CpeSnapshot }) {
  ...
  async function refresh() { setBusy(true); setError(null); try { const next = (await client.execute<NetworkControlState>("network.get")).data; setState(next); setMode(next.networkMode ?? "00"); setLte(selectedFromMask(next.lteBand, LTE_BANDS)); setNr(selectedFromMask(next.nrBand, NR_BANDS)); } catch (cause) { setError(cause instanceof Error ? cause.message : "锁频状态读取失败"); } finally { setBusy(false); } }
  useEffect(() => { void refresh(); }, [client]);
  ...
  async function apply(unlock = false) { if (!state?.networkBand || (!unlock && (lte.length === 0 || (state.nrBand !== null && nr.length === 0 && mode !== "03")))) return; setConfirm(null); setBusy(true); setError(null); setNotice(null); try { await client.execute("network.set", { networkMode: unlock ? "00" : mode, networkBand: state.networkBand, lteBand: unlock ? "7FFFFFFFFFFFFFFF" : bitmask(lte), ...(state.nrBand !== null ? { nrBand: unlock ? "4000000000000000006" : bitmask(nr) } : {}) }); setNotice(unlock ? "已提交自动选网与全频段配置。" : "频段配置已提交；正在等待设备重新选网。" ); await refresh(); } catch (cause) { setError(cause instanceof Error ? cause.message : "锁频失败"); setBusy(false); } }
  return <>
    ...
    <section className="soft-panel"><div className="section-heading"><div><p className="eyebrow">Band lock</p><h2>自定义锁频</h2></div><button className="soft-button" type="button" disabled={busy} onClick={() => void refresh()}>刷新</button></div>
      <label className="input-label" htmlFor="network-mode">首选网络</label><select id="network-mode" value={mode} onChange={(event) => setMode(event.target.value)}><option value="00">自动</option><option value="03">4G Only</option><option value="08">5G 优先（候选）</option></select>
      <BandPicker title="4G 频段" prefix="B" bands={LTE_BANDS} selected={lte} onToggle={(band) => toggle(band, lte, setLte)} />
      {state?.nrBand !== null && <BandPicker title="5G 频段" prefix="N" bands={NR_BANDS} selected={nr} onToggle={(band) => toggle(band, nr, setNr)} />}
      <div className="lock-actions"><button className="soft-button" type="button" disabled={busy || !state?.networkBand} onClick={() => setConfirm("unlock")}>恢复自动</button><button className="primary-button" type="button" disabled={busy || !state?.networkBand || lte.length === 0} onClick={() => setConfirm("apply")}>{busy ? "处理中…" : "应用锁频"}</button></div>
      <p className="panel-note">只提交完整的当前配置，保留设备返回的 NetworkBand。5G 选项仅在 H168 返回 NRBand 后启用；设备仍可能拒绝区域固件不支持的组合。</p>{notice && <p className="action-notice">{notice}</p>}<ErrorText value={error} />
    </section>
...
export function ManagedClients({ client }: { client: H168ControlClient }) {
  ...
  async function blockClient() { if (!block) return; const target = block; setBlock(null); setBusy(true); try { await client.execute("clients.block", { macAddress: target.macAddress, hostName: target.name ?? target.hostName ?? "Blocked device" }); await load(); } catch (cause) { setError(cause instanceof Error ? cause.message : "终端断网失败；该 H168 固件可能不支持此过滤端点"); setBusy(false); } }
  const rows = useMemo(() => clients, [clients]);
  return <><section className="soft-panel"><div className="section-heading"><div><p className="eyebrow">Connected devices</p><h2>在线终端</h2></div><button className="soft-button" type="button" disabled={busy} onClick={() => void load()}>刷新</button></div><div className="managed-clients">{rows.map((item, index) => <article key={item.macAddress}><div className="client-icon">{item.manufacturer === "Apple" ? "●" : "◆"}</div><div><h3>{item.name ?? item.hostName ?? `终端 ${index + 1}`}</h3><p>{item.ipAddress ?? "IP 未返回"} · {item.frequency ?? "频段未知"}</p><small>{item.macAddress}</small></div><button className="danger-soft-button" type="button" onClick={() => setBlock(item)}>断网</button></article>)}</div>{!busy && rows.length === 0 && <p className="loading-line">当前没有在线 WLAN 终端</p>}<p className="panel-note">“断网”使用 Huawei MAC 过滤兼容端点，H168 支持情况需首次操作后确认。请勿断开当前用于管理设备的 iPhone。</p><ErrorText value={error} /></section><ConfirmDialog open={block !== null} title="让这个终端断网？" detail={`${block?.name ?? block?.hostName ?? "该终端"} 将加入 Wi-Fi 过滤列表。当前版本尚不能可靠撤销，请确认不是本机。`} confirmLabel="确认断网" danger onCancel={() => setBlock(null)} onConfirm={() => void blockClient()} /></>;
}
